...
public class AServProtocolManager {
   ...
   /*********************************************************************
		...
		while ( tk != null && tk.hasMoreTokens() ){
			...
			try {
				...
				if ( file.isDirectory() ) {
					...
					getClassList(list, file);
				} else if ( checkFile(file, visited)) {
					//logger.trace("JAR {}", file);
					classPath = getClassPath(visited, file);
				}
			} catch( IOException e ) {
				continue;
			}
	...
	private boolean checkFile(File file, Set<String> visited){
		boolean flag = false;
	...
	private void getClassList(Set<String> list, File file){
	...
	private String getClassPath(Set<String> visited, File file){
	...
	private JarFile openJar(Set<String> visited){
	...
	private void checkAlignmentObject(URI uri1, URI uri2, Alignment init){
		try {
			createAlignmentObject(uri1, uri2, init);
	    } catch ( ClassNotFoundException cnfex ) {
		logger.error( "Unknown method", cnfex );
	...
	private void createAlignmentObject(URI uri1, URI uri2, Alignment init) throws ClassNotFoundException, NoSuchMethodException, InstantiationException,
